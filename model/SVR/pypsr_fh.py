...
def autocorrelation_function(data):
    # 给定固定的j值，计算时间间隔为jt的自相关函数
    # R_{xx}(jt) = 1/N(\sum_{i=0}^{N-1}Y_i*Y_{i+jt})
    # 画出这个函数关于t的图像
    r = []
    size = 100
    flag = True
    ...
    for t in np.arange(0, 100):
        r_t = 1/len(data)*(np.sum(data[i] * data[i+t] for i in range(len(data)-t)))
        r.append(r_t)
        if t == 0:
            compare = r_t
        else:
            if flag and compare * (1-math.exp(-1)) > r_t:
                flag = False
                size = t
    print("size: ", size)
    print("compare: ", compare)
    r_ts = pd.DataFrame(r, index=np.arange(0, 100))
    plt.plot(r_ts, color="blue", label='R_t')
    plt.show()
...
if __name__ == '__main__':
    df = pd.read_csv('inttraffic.csv', sep=',', index_col='Time', header=0)
    ...
    # lags = 16
    # dimensions, false_neighbor_pcts = global_false_nearest_neighbors(train, lag=16, min_dims=1, max_dims=20)
    # (lags=1, dimensions=5)
    # (lags=5, dimensions=5)
    # (lags=16, dimensions=4)
    # print("dimensions: ", dimensions)
    # print("false_neighbor_pcts: ", false_neighbor_pcts)

    train_svr = reconstruct(train, 16, 3)
    print(train_svr, train_svr.shape)
